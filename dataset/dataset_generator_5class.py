    ...
    for cls in classes:
        ...
        for file in tqdm(os.listdir(feature_dir)):
            if file.endswith('.h5'):
                ...
                azimuth = file.split("_")[1]
                if int(azimuth) == 30:
                    data_missing.append(salsa_features)
                    gt_missing.append(gt_label)
                else:
                    data.append(salsa_features)
                    gt_labels.append(gt_label)
                
    return data, gt_labels, data_missing, gt_missing
...
if __name__ == "__main__":
    ...
    data , gt , data_2 , gt_2 = create_dataset(feature_upper_dir)
    ...
    missing_fp = os.path.join(dataset_dir, "missing_data.npy")
    np.save(missing_fp , data_2 , allow_pickle=True)
    missing_gt = os.path.join(dataset_dir, "missing_gt.npy")
    np.save(missing_gt, gt_2, allow_pickle=True)
